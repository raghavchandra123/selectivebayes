...
while inp!="stop":
    if inp=="go":
        tree_vec,mol_vec=pickle.load(open("molvec.pk1","rb"))
        tree_vec = create_var(torch.from_numpy(tree_vec), False)
        mol_vec = create_var(torch.from_numpy(mol_vec), False)
        molecule = model.decode(tree_vec, mol_vec, False)
        if molecule is None:
            molecule = "failed"
        print("~"+molecule+"~")
    if inp[:3]=="enc":
        all_vec = model.encode_latent_mean([inp[3:]])
        all_vec=all_vec.cpu().detach().numpy()
        pickle.dump(all_vec,open("encoded.pk1","wb"))
        print("done")
